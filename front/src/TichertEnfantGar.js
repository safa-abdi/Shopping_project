...
function TichertEnafantBoy() {
  ...
  async function getProduit(){
    ...
    let result = await fetch("http://localhost:3000/getProduitParType/TichertBoy",{
      method:"GET"
    }); 
   ...
   async function searchProduit(key){
    console.warn(key)
    let result = await fetch('http://localhost:3002/searchProduit/'+key);
    result= await result.json();
    console.warn(result)
   setData(result)
...
export default TichertEnafantBoy;
